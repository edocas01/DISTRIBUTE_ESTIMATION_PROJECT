...
voronoi(x,y);
figure()
robots{1}.plot(all_markers, color_matrix, true)
...
if sum(C{1} == 1)
	inf_index = find(C{1} == 1);
end
for j = 1:length(C{1})
	[in on] = inpolygon(V(C{1}(j),1),V(C{1}(j),1),circle.Vertices(:,1),circle.Vertices(:,2));
